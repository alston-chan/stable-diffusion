...
def dream(prompt: str, ddim_steps: int, plms: bool, fixed_code: bool, ddim_eta: float, n_iter: int, n_samples: int, cfg_scale: float, seed: int, height: int, width: int, skip_save: bool, skip_grid: bool):
    ...
    if fixed_code:
        start_code = torch.randn(
            [n_samples, opt.C, opt.H // opt.f, opt.W // opt.f], device=device)
...
def translation(prompt: str, init_img, ddim_steps: int, ddim_eta: float, n_iter: int, n_samples: int, cfg_scale: float, denoising_strength: float, seed: int, height: int, width: int, skip_save: bool, skip_grid: bool):
    torch.cuda.empty_cache()
    parser = argparse.ArgumentParser()
    ...
    w, h = map(lambda x: x - x % 32, (width, height))
    image = image.resize((w, h), resample=PIL.Image.LANCZOS)
    print(f"cropped image to size ({w}, {h})")
    image = np.array(image).astype(np.float32) / 255.0
    image = image[None].transpose(0, 3, 1, 2)
                ...
                for n in trange(n_iter, desc="Sampling", disable=not accelerator.is_main_process):
                    for prompts in tqdm(data, desc="data", disable=not accelerator.is_main_process):
                        ...
                        noise = torch.randn_like(
                            x0) * sigmas[ddim_steps - t_enc - 1]  # for GPU draw
                        xi = x0 + noise
...
dream_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(
            placeholder="A corgi wearing a top hat as an oil painting.", lines=1),
        gr.Slider(minimum=1, maximum=150, step=1,
        ...
        gr.Slider(minimum=0.0, maximum=1.0, step=0.01,
                  label="DDIM ETA", value=0.0, visible=False),
        gr.Slider(minimum=1, maximum=50, step=1,
                  label='Sampling iterations', value=2),
        gr.Slider(minimum=1, maximum=8, step=1,
                  label='Samples per iteration', value=1),
        gr.Slider(minimum=1.0, maximum=15.0, step=0.5,
                  label='Classifier Free Guidance Scale', value=7.0),
        gr.Number(label='Seed', value=-1),
        gr.Slider(minimum=32, maximum=2048, step=32,
                  label="Height", value=512),
        gr.Slider(minimum=32, maximum=2048, step=32, label="Width", value=512),
        gr.Checkbox(label='Skip Save', value=True),
        gr.Checkbox(label='Skip Grid Save', value=True)
    ],
    outputs=[
        gr.Gallery(),
...
img2img_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Image(value="https://raw.githubusercontent.com/CompVis/stable-diffusion/main/assets/stable-samples/img2img/sketch-mountains-input.jpg",
        ...
        gr.Slider(minimum=1, maximum=50, step=1,
                  label='Sampling iterations', value=2),
        gr.Slider(minimum=1, maximum=8, step=1,
                  label='Samples per iteration', value=2),
        gr.Slider(minimum=1.0, maximum=15.0, step=0.5,
                  label='Classifier Free Guidance Scale', value=7.0),
        gr.Slider(minimum=0.0, maximum=1.0, step=0.01,
                  label='Denoising Strength', value=0.75),
        gr.Number(label='Seed', value=-1),
        gr.Slider(minimum=64, maximum=2048, step=64,
                  label="Resize Height", value=512),
        gr.Slider(minimum=64, maximum=2048, step=64,
                  label="Resize Width", value=512),
        gr.Checkbox(label='Skip Save', value=True),
        gr.Checkbox(label='Skip Grid Save', value=True)
    ],
    outputs=[
...
demo = gr.TabbedInterface(interface_list=[
                          dream_interface, img2img_interface], tab_names=["Dream", "Image Translation"])
